...
本文是基于 import table 和 Wine 源码映射做的静态缺口分析，适合做 backlog 和优先级参考；但当前 runtime 进度已经前进，先补充最新状态：

### 0.1 当前运行状态

1. `cpuz_arm64.exe` 已不再停留在早期 unresolved import 阶段。
2. `shell32` 已纳入仓库源码和构建链，`SHGetMalloc` 不再是当前阻塞。
3. `wininet/urlcache` 使用的 file-backed section 已补上最小 writeback 路径，`INetCookies/index.dat` 可正确初始化并持久化。
4. 在清理旧坏缓存目录后，最近验证中 `cpuz` 可运行到：
   - `PROCESS_EXIT: code=0`

### 0.2 对本文结论的正确使用方式

因此，下面的 `P0/P1/P2` 不应理解为“程序今天仍然卡在这里”，而应理解为：

1. 这些是从静态导入面推导出的系统能力需求。
2. 其中一部分已经被实现或绕过。
3. 剩余部分仍可作为后续完善 syscall / win32k / DLL 语义时的候选 backlog。

### 0.3 当前仍然成立的工程判断

即使当前 `cpuz` 已能跑通到退出，下面两点仍然成立：

1. `USER32/GDI32/win32k` 依赖依旧是 CPU-Z 的主依赖面，而不是边角路径。
2. 继续推进时，必须坚持“Wine DLL 语义 + guest kernel syscall/win32k 能力”这两条线一起补，不能只盯着单个 Nt syscall。
