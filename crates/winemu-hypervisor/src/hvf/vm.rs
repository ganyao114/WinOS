...
impl Vm for HvfVm {
    fn map_memory(&self, gpa: Gpa, hva: *mut u8, size: usize, prot: MemProt) -> Result<()> {
        ...
        if prot.contains(MemProt::READ)  { flags |= ffi::HV_MEMORY_READ; }
        if prot.contains(MemProt::WRITE) { flags |= ffi::HV_MEMORY_WRITE; }
        if prot.contains(MemProt::EXEC)  { flags |= ffi::HV_MEMORY_EXEC; }

        let ret = unsafe { ffi::hv_vm_map(hva, gpa.0, size, flags) };
        if ret != ffi::HV_SUCCESS {
            return Err(WinemuError::Hypervisor(format!("hv_vm_map failed: {:#x}", ret)));
        }
        Ok(())
    }
    ...
    fn unmap_memory(&self, gpa: Gpa, size: usize) -> Result<()> {
        ...
        if ret != ffi::HV_SUCCESS {
            return Err(WinemuError::Hypervisor(format!("hv_vm_unmap failed: {:#x}", ret)));
        }
        Ok(())
    }
