...
impl DataDir {
    pub fn is_present(self) -> bool { self.rva != 0 && self.size != 0 }
}
...
impl PeHeaders {
    ...
    pub unsafe fn parse(data: *const u8, len: usize) -> Result<Self, PeError> {
        if len < 64 { return Err(PeError::TooSmall); }
        if ru16(data) != MZ_MAGIC { return Err(PeError::InvalidMz); }
        let lfanew = ru32(data.add(60)) as usize;
        if lfanew + 24 > len { return Err(PeError::InvalidPe); }
        if ru32(data.add(lfanew)) != PE_MAGIC { return Err(PeError::InvalidPe); }
        ...
        if oh_off + 2 > len { return Err(PeError::InvalidPe); }
        if ru16(oh) != OPT_MAGIC_PE32PLUS { return Err(PeError::NotPe32Plus); }
        if oh_off + 96 > len { return Err(PeError::TooSmall); }
    ...
    pub fn data_dir(&self, idx: usize) -> Option<DataDir> {
        if idx >= self.num_dirs as usize { return None; }
        let off = self.oh_off + 96 + idx * 8;
        if off + 8 > self.len { return None; }
        unsafe {
            let p = self.base.add(off);
            Some(DataDir { rva: ru32(p), size: ru32(p.add(4)) })
        }
    }

    /// 读取第 i 个节头
    pub fn section(&self, i: usize) -> Option<SectionHeader> {
        if i >= self.num_sections as usize { return None; }
        let off = self.sec_off + i * 40;
        if off + 40 > self.len { return None; }
        unsafe {
    ...
    pub fn sections(&self) -> SectionIter<'_> {
        SectionIter { headers: self, idx: 0 }
    }
