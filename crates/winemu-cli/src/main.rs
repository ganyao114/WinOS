...
fn main() -> Result<()> {
    ...
    let kernel_image = std::fs::read("winemu-kernel.bin")
        .context("failed to read winemu-kernel.bin")?;

    let syscall_table_toml = std::fs::read_to_string(&syscall_table_path)
        .with_context(|| format!("failed to read syscall table: {}", syscall_table_path.display()))?;

    let hypervisor = winemu_hypervisor::create_hypervisor()
...
fn parse_args(args: &[String]) -> Result<(PathBuf, PathBuf, PathBuf, Vec<PathBuf>)> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--dll-path" => {
                i += 1;
                dll_paths.push(PathBuf::from(args.get(i).context("--dll-path requires a path")?));
                i += 1;
            }
            arg => {
    ...
    let exe = exe.context("usage: winemu run [--syscall-table <path>] [--root <path>] [--dll-path <path>] <exe>")?;
