...
static void fill_pattern(uint8_t* dst, uint32_t len, uint32_t round) {
    uint32_t i;
    for (i = 0; i < len; i++) {
        dst[i] = (uint8_t)((i + round * 17U) ^ 0x5AU);
    }
}

void mainCRTStartup(void) {
    /* Hostcall broker async threshold for READ/WRITE is 128KB. */
    ...
    int64_t wait_timeout_short_100ns = -1000;       /* 100us */
    NTSTATUS st;
    uint32_t i;
    uint32_t round;
    uint32_t len = (uint32_t)sizeof(write_buf);

    write_str("== syscall_hostcall_async_test ==\r\n");

    init_unicode(&path_us, path_buf, "guest/sysroot/hostcall_async_test.bin");
    init_oa(&oa, &path_us);
    st = NtCreateFile(
        &file_handle,
    ...
    check("NtCreateFile(hostcall async file) returns STATUS_SUCCESS", st == STATUS_SUCCESS);
    check("NtCreateFile(hostcall async file) returns valid handle", file_handle != 0);
    ...
    st = NtWaitForSingleObject(io_event, 0, &wait_timeout_short_100ns);
    check("Event starts unsignaled (immediate timeout)", st == STATUS_TIMEOUT);

    for (round = 0; round < 2; round++) {
        uint64_t offset = ((uint64_t)round) * ((uint64_t)len);
        fill_pattern(write_buf, len, round);
        for (i = 0; i < len; i++) {
            read_buf[i] = 0;
        }
        ...
        check("async read/write payload matches", i == len);

        st = NtWaitForSingleObject(io_event, 0, &wait_timeout_short_100ns);
        check("Event auto-resets after completion wait", st == STATUS_TIMEOUT);
    }
    ...
    write_str("syscall_hostcall_async_test summary: pass=");
    write_str(" fail=");
    write_str("\r\n");
