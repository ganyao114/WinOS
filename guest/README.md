...
`guest/sysroot/` contains generated guest binaries (for example `ntdll.dll`, `win32u.dll`, `msvcrt.dll`, guest test EXEs).

Layout:
- `guest/ntdll/`: ntdll implementation and build scripts.
- `guest/win32u/`: win32u source-reuse build (Wine sources mirrored directly in this folder).
- `guest/msvcrt/`: msvcrt source-reuse build (Wine sources mirrored directly in this folder).
